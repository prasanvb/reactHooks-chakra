...
const countResolver = (currentState, action) => {
  switch (action.type){
    case 'increment':
      return {...currentState, counterInitialValue: currentState.counterInitialValue + action.value}
    case 'decrement': 
      return {...currentState, counterInitialValue: currentState.counterInitialValue - action.value}
    case 'reset': 
      return initialState
...
const UseReducerGlobalStateManagement = () => {
  ...
  return (
    <userCounter.Provider value={{counter: count, dispatcher: dispatch}}>
      <Container m={5} p={5}>
        <Text fontSize={20} textDecoration="underline"> Global State Management using useReducer and useContext </Text>
        <p>Counter = {count.counterInitialValue} </p>
        <Button onClick={()=> dispatch({type: "increment", value: 1})} m={2}>Increment</Button>
        <Button onClick={()=> dispatch({type: "decrement", value: 1})} m={2}>Decrement</Button>
        <Button onClick={()=> dispatch({type: "reset"})} m={2}>Reset</Button>
