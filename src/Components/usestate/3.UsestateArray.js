...
const UsestateArray = () => {
  ...
  const addItemsToArray = () => {
    setItemsArray((old) => { return [...old, {id: itemsArray.length, value: Math.floor(Math.random()*10000)} ]})
  }
  console.log(itemsArray);

  return (
    <Container m={5} p={5}>
      <Text fontSize={20} textDecoration="underline">useState Array Example 3</Text>
      <Container m={5} p={5} border="1px" borderColor="grey">
        { fruitsArray.map(ind => (<p key={ind}> {ind} </p>)) }
      </Container>
      <Container m={5} p={5} border="1px" borderColor="grey">
